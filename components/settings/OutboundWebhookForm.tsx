...
import { saveOutboundWebhook, sendTestPing } from "@/app/actions/outboundWebhook";
...
export default function OutboundWebhookForm({ initial }: { initial: Initial }) {
  ...
  function save(e: React.FormEvent) {
    e.preventDefault();
    startSaving(async () => {
      const res = await saveOutboundWebhook({ url, secret, active });
    ...
    <div className="flex flex-col gap-4">
      <form onSubmit={save} className="flex flex-col gap-3 rounded-xl border border-border bg-card p-4">
        <div className="flex flex-col gap-1">
          ...
          <Input id="url" value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://prod-00.westus.logic.azure.com/... (or http://localhost:3200/api/dev/echo to test locally)" />
        </div>
        <div className="flex flex-col gap-1">
